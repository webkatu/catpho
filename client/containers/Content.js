...
import NavigationButton from '../components/CatContent/NavigationButton.js'
...
class CatContent extends React.Component {
	componentWillMount() {
		if(this.props.contentIds.length === 0) return;
		
		this.props.dispatch(actions.startGalleryMode({
	...
	handleCloseButtonClick(e) {
		this.props.dispatch(actions.closeCatContentViewer());
	}

	handlePreviousButtonClick(e) {
		e.preventDefault();
		if(this.props.catContent.isFetchingContent) return;
		if(this.props.catContent.isGalleryMode) {
			this.props.dispatch(actions.goPreviousContent());
		}else {
			this.context.router.push(e.target.pathname);
		}
		this.props.dispatch(actions.fetchContent(this.props.catContent.previousContentId));
	}
	...
	handleNextButtonClick(e) {
		...
		if(this.props.catContent.isFetchingContent) return;
		if(this.props.catContent.isGalleryMode) {
			this.props.dispatch(actions.goNextContent());
		}else {
			this.context.router.push(e.target.pathname);
		}
		this.props.dispatch(actions.fetchContent(this.props.catContent.nextContentId));
	}
	...
	handlePostCommentTextChange(e) {
		this.props.dispatch(actions.inputPostCommentText(e.value));
	}

	handleCommentFormSubmit(e) {
		e.preventDefault();
		this.props.dispatch(actions.postComment(e.target));
	}
	...
	render() {
		...
		const tagNodes = content.tags.map((tag) => {
			return (
				<span>
					<a href={'/content/' + tag} onClick={::this.handleAnchorClick}>{tag}</a>
					{' '}
				</span>
			);
		});
		...
		return (
			<article className="catContent">
				...
				<FavoriteButton
					...
					disabled={catContent.isRequestingFavorite}
				...
				{commentBoxNode}
				<NavigationButton
					hasContent={catContent.hasPreviousContent}
					contentId={previousContentId}
					onClick={::this.handlePreviousButtonClick}
				>&lt;</NavigationButton>
				<NavigationButton
					hasContent={catContent.hasNextContent}
